    ...
    function lock(
        ...
        for(uint256 i = 0; i <= _token.length; i++) {
            if (_token[i] != address(0)) {
                require(IERC20(_token[i]).balanceOf(msg.sender) >= _tokenAmount, "Not enough funds!");
                require(IERC20(_token[i]).allowance(msg.sender, address(this)) >= _tokenAmount, "Have no approve!");
